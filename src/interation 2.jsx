...
const HOUR_WEIGHTS = [1.0, 1.1, 1.2, 1.1];
const weightedAvg = (vals) => {
  const total = HOUR_WEIGHTS.reduce((a, b) => a + b, 0);
  return vals.reduce((sum, v, i) => sum + v * HOUR_WEIGHTS[i], 0) / total;
};
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      ...
      >
        ...
        >
          <div style={{ ...S.hourCell, color: "#1a9e6e" }}>Wtd Avg</div>
          <div style={{ ...S.hourCellVal, color: "#1a9e6e" }}>
            {weightedAvg(hours.map((h) => h.wind)).toFixed(1)}
          </div>
          <div style={{ ...S.hourCellVal, color: "#1a9e6e" }}>
            {weightedAvg(hours.map((h) => h.gusts)).toFixed(1)}
          </div>
          <div style={{ ...S.hourCellVal, color: "#1a9e6e" }}>
            {weightedAvg(hours.map((h) => h.precip)).toFixed(2)}
          </div>
          <div style={{ ...S.hourCellVal, color: "#1a9e6e" }}>
            {Math.round(weightedAvg(hours.map((h) => h.temp)))}°
          </div>
        </div>
      </div>
    </div>
...
export default function App() {
  ...
  const fetchWeather = useCallback(async () => {
    ...
    setFetching(true);
    setHourlyData(null);
    setLiveWeather(null); // CRITICAL: Clear old ghost locations on new search
    setWeatherStatus({ msg: "Finding location...", cls: "loading" });

    try {
      // 1. Parse user input to separate city and state
      const parts = loc.split(",");
      const searchCity = parts[0].trim();

      // Ask Open-Meteo for the top 10 matches for that city name
      ...
      let match = geoData.results[0]; // Default to the most populated match

      // If the user included a state/region (like "OR" or "Oregon"), find the exact match in the top 10
      if (parts.length > 1) {
        const searchRegion = parts[1].trim().toLowerCase();

        // Dictionary to translate abbreviations to full names so the API can read them
        const states = {
          al: "alabama",
          ak: "alaska",
          az: "arizona",
      ...
      // 2. Fetch Hourly Weather using coordinates
      ...
      // 3. Slice the 4 hours starting from the tee time
      ...
      for (let i = 0; i < 4; i++) {
        ...
        if (hourIdx < 24) {
          hoursArray.push({
            label: `Hr ${i + 1}`,
            time: `${hourIdx}:00`,
            wind: Math.round(weatherData.hourly.wind_speed_10m[hourIdx]),
            gusts: Math.round(
              weatherData.hourly.wind_gusts_10m[hourIdx] ||
                weatherData.hourly.wind_speed_10m[hourIdx] * 1.4
            ),
            precip: weatherData.hourly.precipitation[hourIdx],
            temp: Math.round(weatherData.hourly.temperature_2m[hourIdx]),
          });
        }
      }
      ...
      // Apply the values to the manual input fields
      setWind(weightedAvg(hoursArray.map((h) => h.wind)).toFixed(1));
      setGusts(weightedAvg(hoursArray.map((h) => h.gusts)).toFixed(1));
      setPrecip(weightedAvg(hoursArray.map((h) => h.precip)).toFixed(2));
      setTemp(String(Math.round(weightedAvg(hoursArray.map((h) => h.temp)))));

      setHourlyData(hoursArray);

      // Update UI with confirmed location
      ...
      <div style={S.main}>
        ...
        <div style={S.card}>
          ...
          >
            ...
            hrs) and applies weighted averages. Wind tends to build mid-round,
            so later hours are weighted slightly higher.
          </div>
          ...
          >
            {hourlyData ? "Weighted Round Averages (editable)" : "Manual Entry"}
          </div>
          <div style={S.grid4}>
            <Field
...
function Results({ r, hourlyData, teeTime }) {
  ...
  const oddsText =
    ...
    (r.usedHourly ? `Conditions: 4-hr weighted avg — ` : `Conditions: `) +
  ...
  const formulaText =
    ...
    (r.usedHourly
      ? `[4-hour weighted avg used — weights: Hr1×1.0, Hr2×1.1, Hr3×1.2, Hr4×1.1]\n`
      : "") +
    ...
    <div>
      ...
      <div style={{ ...S.card, marginBottom: 8 }}>
        ...
        {r.usedHourly && (
          ...
          >
            ↑ Based on 4-hour weighted forecast from tee time{" "}
      ...
      <div style={S.disclaimer}>
        ...
        hours weighted [1.0, 1.1, 1.2, 1.1] to reflect wind typically building
        mid-round. Simulated PCC replicates WHS Rule 5.6 intent. Wind
        coefficients from peer-reviewed research (~1.15 strokes/8.6 mph elite;
        scaled for amateurs). For entertainment only — not an official GHIN
        calculation.
      </div>
    </div>
  );
